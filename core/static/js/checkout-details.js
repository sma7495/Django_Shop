...
$(document).ready(function() {
    $('#toggle-form').click(function() {
        $('#delivery-info').slideToggle();
    });
});

$(document).ready(function() {
    $('.discount-code-link').click(function() {
        $('.discount-code-box').slideToggle();
    });
});
...
const product_shopping_cart=document.querySelectorAll(".product-shopping-cart");
const count_cart=product_shopping_cart.length;
const count_cart_class=document.querySelector(".count-cart");
count_cart_class.innerText=count_cart;

document.addEventListener('DOMContentLoaded', function() {
    const  checkout=document.querySelector(".checkout");
    const stickycheckoutClass="sticky-checkout";
    const checkoutOffset = checkout.offsetTop;
    document.addEventListener("scroll",function (){


        const scrollTop = window.scrollY;
        if (scrollTop >= checkoutOffset) {
            checkout.classList.add(stickycheckoutClass);
        }
        else {
            checkout.classList.remove(stickycheckoutClass);
        }
...
function calculateProgress(position, totalTabs) {
    return position > 0 ? (position / (totalTabs - 1) * 100) : 0;
}
...
function tabEventShow(event) {
    ...
    const totalTabs = list.length;


    const progressWidth = calculateProgress(position, totalTabs);
...
progressTab.style.width = '0%';

function selectDay(dayElement) {

    const selected = document.querySelector('.tickBox.selected');
    if (selected) {
        selected.classList.remove('selected');
        selected.querySelector('.tick').style.display = 'none';
    }


    dayElement.classList.add('selected');
    dayElement.querySelector('.tick').style.display = 'block';
}
