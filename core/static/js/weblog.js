...
const container_video = document.getElementById('video-blog'),
    thumb_video = document.querySelectorAll('.blog-video-frame');


thumb_video.forEach(item => {
    item.addEventListener('click', () => {
        const videoSrc = item.children[0].children[0].dataset.video;
        console.log(videoSrc)
        const posterSrc = item.children[0].children[0].src;

        container_video.src = videoSrc;
        container_video.setAttribute('poster', posterSrc); // اضافه کردن poster

        thumb_video.forEach(playing => playing.classList.remove('playing'));
