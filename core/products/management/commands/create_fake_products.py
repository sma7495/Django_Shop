...
from django.core.files import File
from django.contrib.auth import get_user_model
from ...models import Product, ProductCategory, ProductColor, ProductGuarantee  # Update with your actual model imports
import os
from django.conf import settings
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not colors.exists():
            self.stdout.write(self.style.ERROR('No colors found. Please create some colors first.'))
            return
            
        # Get some guarantees
        guarantees = ProductGuarantee.objects.all()
        if not guarantees.exists():
            self.stdout.write(self.style.ERROR('No guarantees found. Please create some guarantees first.'))
            return
        ...
        for i in range(count):
            ...
            product_data = {
                ...
                'discount_percent': random.choice([0, 0, 0, 5, 10]),  # Mostly no discount
                'status': random.choice(['draft', 'published']),
                'guarantee': random.choice(guarantees) if guarantees.exists() else None
            }
            ...
            try:
                ...
                # Add random colors (1-3 colors per product)
                num_colors = random.randint(1, 3)
                product.color.add(*random.sample(list(colors), num_colors))
